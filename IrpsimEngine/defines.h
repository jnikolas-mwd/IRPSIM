...
public:
	...
	CMDefinition(const string& n,const string& v) : CMIrpObject(n) {value=_wtof(v.c_str());}
	CMDefinition(const string& n, double v) : CMIrpObject(n), value(v) {}
	CMDefinition(const string& n, const string& v, int app_id) : CMDefinition(n,v) { this->app_id = app_id; }
...
public:
	static CMDefinition* Find(const string& n, unsigned short* loc = 0);
	static void Add(const string& n,const string& v, int app_id) {definition_array.Add(new CMDefinition(n,v,app_id));}
	static double GetValue(const string& n) {CMDefinition* d = Find(n);return d ? d->GetValue() : 0;}
	static int IsDefined(const string& n,unsigned short* loc=0) {return Find(n,loc) ? 1 : 0;}
	static unsigned short GetIndex(const string& n) {unsigned short loc;Find(n,&loc);return loc;}
    static void Reset() {definition_array.ResetAndDestroy(1);}
	static double GetDefinition(const string& n);
